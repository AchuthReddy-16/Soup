...
tests/                - Test suite (371 files, 18184 tests)
...
| test_issue320_lazy_callback_import.py | #320 — eleven TrainerCallback modules deferred class construction via PEP 562 `__getattr__`; each callback is still a real `TrainerCallback` subclass (#308 property); each inherits no-op event stubs; the lazy class is cached on repeated access. |
